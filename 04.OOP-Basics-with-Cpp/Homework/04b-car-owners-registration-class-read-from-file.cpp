...
Task 4B
 Write a program which reads several Person objects and car registrations from a file. Test if
 the objects you created when reading from the file are correct by writing functions which
 assert the information stored in the objects (a good way to do that would be to have getInfo()
 methods on the Person and car registration classes and assert if that info is correct).
...
public:
    ...
    static void restoreAllDataFromFile(const string & file_name)
    {
        ...
        {
            getline(fs >> ws, line);
            personsList.push_back(parsePersonFromString(line));
        }

        int registrations, horsepower, id;
        string registration_number, manufacturer, model;
        fs >> registrations;

        for (int i = 0; i < registrations; i++)
        {
            getline(fs >> ws, registration_number);
            getline(fs >> ws, manufacturer);
            getline(fs >> ws, model);
            fs >> horsepower >> id;

            int index = findPersonIndexByID(id);
            if (index != -1) // There's a person with that ID
            {
                CarsRegister::addCarRegistration(manufacturer, model, horsepower, &getPersonByIndex(index), registration_number);
            }
            else
            {
                throw "No valid person found to restore registration";
            }
        }
    }

    static Person * parsePersonFromString(const string & person_data)
    {
        stringstream ss(person_data);
        int id, age;
        string name;
        ss >> id >> age;
        ...
        int index = findPersonIndexByID(id);
        if (index == -1) // No active Person with that ID found
        {
        ...
        {
            throw "Person with that ID already exist";
        }
    }
};
...
void testCreateSaveRestore()
{
    cout << "Clear all data... ";
    CarsRegister::clearGlobalData();

    cout << "done" << endl
        << "Show empty database:" << endl
        << CarsRegister::getSystemInfo() << endl
        << "Create 10 random Persons...";
    CarsRegister::generatePersons(10);

    cout << "done" << endl
        << "Create 10 random Registrations... ";
    CarsRegister::generateCarRegistrations(10);

    cout << "done" << endl
        << "Show current database state:" << endl
        << CarsRegister::getSystemInfo()
        << "Save current database to db.txt... ";
    CarsRegister::saveAllDataToFile("db.txt");
    cout << "done" << endl
        << "Save current database to memory string for reference...";

    string database_persons_string = CarsRegister::getPersonsInfo();
    string database_registrations_string = CarsRegister::getCarsInfo();

    cout << "done" << endl << "Clear database...";
    CarsRegister::clearGlobalData();
    ...
    cout << "Restore database from db.txt... ";
    CarsRegister::restoreAllDataFromFile("db.txt");

    cout << "done" << endl
        << "Show current database state:" << endl
        << CarsRegister::getSystemInfo() << endl
        << "Restored persons and snapshot are equal... ";

    bool string_match = database_persons_string.compare(CarsRegister::getPersonsInfo()) == 0;
    cout << boolalpha << string_match << endl
        << "Restored registrations and snapshot are equal... ";
    string_match = database_registrations_string.compare(CarsRegister::getCarsInfo()) == 0;
    cout << boolalpha << string_match << endl << endl
        << "Test done!" << endl << endl;
}
            ...
            case 15:
                {
                    ...
                    cout << "Enter file name: ";
                    getline(cin >> ws, file_name);
                    CarsRegister::restoreAllDataFromFile(file_name);
                }   break;
            case 16:
                {
